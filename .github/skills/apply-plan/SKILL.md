...
A plan folder under `tmp/` — e.g. `tmp/storage-s3-multipart/`. If the caller does not name one, list candidates and ask which to apply:
...
- **Validate the affected crates** with the `validate` skill (make/cargo, scoped) — do not proceed to the next step on a red one.
...
Every step is executed against rskit's engineering baseline, not a looser plan-local standard. After a step (or a coherent group of steps) lands, run the `review` skill's passes over the diff in a fresh, clean-context agent. Treat a green `validate` run as necessary but not sufficient.
