...
- **Readable files & injected composition.** Split by concern into focused files; when you touch a file that has grown over-long or mixes distinct concerns, promote it to a folder (declare-only `mod.rs` + concern-named submodules, `#[cfg(test)] test_support` for shared fixtures) **in this step** — do not defer the reorg. Structure work travels with the change; only split where it genuinely improves discoverability, and leave cohesive single-concern files alone. Inject registries/config; no import-time side effects or mutable global registries.
