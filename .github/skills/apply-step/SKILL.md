...
Apply the current step's actions **test-first**, honoring rskit's engineering baseline —
...
- **Placement & layering.** Right crate (`core/rskit-<name>` vs `contrib/<domain>/<name>`);
  acyclic dependencies (lower crates never depend on higher);
  new crates carry `#![warn(missing_docs)]` and are wired into the workspace + facade.
- **Canonical reuse.** Before writing a new type/helper,
  ...
  Put new logic in concern-named modules; never in `lib.rs`/`mod.rs`.
...
- **Validate** the affected crates with the `validate` skill (make/cargo, scoped),
  green under race/shuffle/parallel. A step does not land red.
  Run `make structure` (declare-only aggregator guard) and keep new aggregators clean.
