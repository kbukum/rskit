...
A commit message tells the next developer *what changed and why*, in as few words as it takes. This skill keeps rskit's history tidy: one focused change per commit, a message that reads like a maintainer wrote it, and **nothing extra** — no trailers, no attributions, no process log.
...
rskit uses **Conventional Commits** (`feat`, `fix`, `docs`, `refactor`, `test`, `chore`). A single compact subject line, imperative mood, describing the change **as it now stands**:
...
- Pick the crate/scope that matches the surrounding history; don't force a scope that isn't real.
