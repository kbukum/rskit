...
- **Prefix with your username**, then a short kebab-case summary of the change: `<username>/<short-change-summary>` (e.g. `kbukum/harden-process-argv-validation`).
- Name by the actual change: the crate/capability touched and the outcome (`kbukum/typestate-app-lifecycle`, `kbukum/storage-s3-multipart`).
