...
- **Module cohesion & file-level structuring.** The inverse of the aggregator rule: a single file that has grown **over-long**, or that **mixes several distinct concerns**, should be promoted to a folder — a declare-only `mod.rs` plus concern-named submodules (the `http/`, `discovery/`, `apikey/` shape), with shared test fixtures in a `#[cfg(test)] test_support` module. Reorg is **not only** about one over-long file: when a single module/crate has accumulated many loosely-related files (roughly >5–10, excluding `test_support`/`tests`) that fall into groups which are **not tightly coupled**, each cohesive group should move into its own concern-named submodule folder (nested `mod.rs`), so the reader lands in the right file *and folder* from the layout. This is **criteria-driven, not a line count**: reorganize only where the split genuinely improves discoverability and maintainability, or where a parent already holds many organizable sibling files. A cohesive, single-concern file (or a flat set of files that all serve one concern) is fine at any length — do not split for the sake of it. When a change touches such a file, restructure it **in the same change** rather than deferring the reorg (deferred reorg is how structure debt accumulates). Flag an over-long, concern-mixed file left un-split as a should-fix.
