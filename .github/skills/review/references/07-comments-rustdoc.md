...
**Scope note.** *Changes mode:* review every comment and rustdoc touched by (or owed by) the diff, including comments left stale by a code change. *Project mode:* sweep all prose across `core/*/src`, `core/*/tests`, `contrib/*/*/src`, `contrib/*/*/tests`, and any other first-party source — crate headers (`//!`), item docs (`///`), and inline (`//`) comments alike. This pass is about prose, not code; never weaken a check from another pass to make a comment "true".
...
- **Self-contained and durable.** Understandable from the code alone, and still true a year from now regardless of which branch, PR, or release shipped it.
- **rustdoc is for the reader of the API.** Crate (`//!`) and item (`///`) docs describe the contract: what it is, what it guarantees, how to use it, when it errors or panics. Every public item carries docs (`#![warn(missing_docs)]` is satisfied) and documents `# Errors` (and `# Panics` where any panic is reachable) per the crate's conventions. A new crate's `//!` header states its purpose and its place in the toolkit. Doc examples compile and reflect real usage.
- **SAFETY comments are real.** Each `unsafe` block's `// SAFETY:` comment states the actual invariant that makes it sound, not a placeholder — this pass verifies the prose, pass `04` verifies the gate.
- **Domain vocabulary, not project trivia.** Architecture and domain terms that appear in the code or docs (e.g. crate names, port/seam names, typestate phase names, `Component`/`Registry`/`Provider` concepts that are real types) are fine. They describe the system, not the project's history.
...
Then the gate: `make doc` (`-D warnings`) must pass, and `cargo fmt --check` stays clean. Scope the doc build with `make doc C=<crate>` in changes mode.
