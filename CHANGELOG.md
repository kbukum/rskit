...
- Add `rskit_util::hash`: a domain-free BLAKE3 content-hashing helper (`ContentHasher` plus the one-shot `hash_hex`) rendering a stable lowercase-hex digest, with `update_framed` for `label:value\0` domain separation so independently folded fields cannot collide.
