...
- Add `rskit-workload`, a provider-based container/pod **orchestration** layer (facade feature `workload`): a minimal typed `Manager` trait (`deploy`/`stop`/`remove`/`wait`/`logs`/`list`) with opt-in `ExecCapable`/`StatsCapable`/`LogStreamer`/`EventWatcher`/`SystemInfoCapable`/`DiskUsageCapable`/`ImageInspector`/`ImageEventWatcher` capability traits, `DeployRequest`/`ResourceConfig`/`NetworkConfig` specs, `WorkloadState`/`RestartPolicy` state, and typed `WorkloadStatus`/`WorkloadStats`/`WorkloadEvent` reports. Backend selection is config-driven through `WorkloadConfig::provider` and an injected `WorkloadRegistry`/`ManagerFactory`; no backend is registered by default and importing the crate has no side effects. `WorkloadComponent` binds the resolved manager to the `rskit-component` lifecycle. This is distinct from `rskit-worker` (in-process pools/scheduling) — it orchestrates external workloads such as containers.
