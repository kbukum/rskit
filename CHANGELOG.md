...
- Add `rskit_util::glob` (`glob_match`, `has_wildcard`, and the reusable `Glob`): dependency-free shell-style `*`/`?` matching over single string segments with no path or separator semantics, so it composes into identifiers, names, or topic segments. Matching is Unicode-`char`-correct (`?` matches exactly one scalar value, not one byte) and case-sensitive, using a linear-time two-pointer scan with constant backtracking state so untrusted patterns cannot trigger exponential blow-up. `Glob` compiles a pattern once and remembers whether it is a plain literal for reuse across a candidate set.
