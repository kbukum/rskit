...
- Keep dependencies up to date in the relevant split workspace and run `make deny` before shipping dependency changes.
...
- The Rust toolchain is pinned via `rust-toolchain.toml`; CI enforces this invariant. Split workspace lockfiles are committed and checked via locked Cargo operations.
...
- Supply-chain policy is enforced by [`cargo-deny`](https://embarkstudios.github.io/cargo-deny/) via [`deny.toml`](deny.toml): banned crates, license allowlist, advisory database, and source allowlist.
