...
use rskit_media::{
    ...
    ops::{
        ...
        ImageFormat, InterpolateConfig, InterpolateModel, MediaOp, MixAudioOp, OverlayOp,
        OverlayPosition, PadOp, ResizeMode, ResizeOp, Rotation, SceneDetectConfig, SubtitleFormat,
...
fn non_path_sources_are_read_from_stdin_pipe() {
    ...
    let cmd =
        FfmpegCommand::compile(&source, &[], None, &default_config(), &default_registry()).unwrap();
...
fn transcode_quality_bitrate_speed_and_streaming_variants_compile() {
    for (quality, crf) in [
        ...
        let output = OutputConfig::new(Format::new("mkv"))
            .with_video(VideoSettings::new(Codec::new(video_codec::H265)).with_quality(quality));
        let args = compile_args(&[MediaOp::Transcode(output)]);
        assert!(
    ...
    let audio_constrained = compile_args(&[MediaOp::Transcode(
        OutputConfig::new(Format::new("mp3")).with_audio(
            AudioSettings::new(Codec::new(audio_codec::MP3)).with_bitrate(Bitrate::Constrained {
                target: 96_000,
                max: 128_000,
            }),
        ),
    ...
    let dash = compile_args(&[MediaOp::Transcode(
        OutputConfig::new(Format::new("mp4")).with_streaming(StreamingConfig::Dash(DashConfig {
            segment_duration: 6,
            use_template: true,
            use_timeline: true,
        })),
    ...
    let rtmp = compile_args(&[MediaOp::Transcode(
        OutputConfig::new(Format::new("flv")).with_streaming(StreamingConfig::Rtmp(RtmpConfig {
...
fn burn_subtitles_operation_compiles_through_command_dispatch() {
    let subtitles =
        rskit_media::subtitle::SubtitleTrack::new().add(TimeRange::from_seconds(0.0, 1.0), "hello");
