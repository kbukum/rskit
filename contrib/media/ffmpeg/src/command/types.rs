...
///
/// When available (e.g., from a prior probe),
/// these hints let the command builder generate more accurate filter graphs. Without hints,
/// the builder uses conservative defaults.
#[derive(Debug, Clone, Default)]
pub(crate) struct SourceHints {
    /// Whether the primary source has at least one audio stream. `None` means unknown —
    /// the builder will assume audio exists (common case).
    pub has_audio: Option<bool>,
}

/// Compiled FFmpeg command ready for execution.
///
/// Holds all inputs, filters, output options, and global flags.
/// Use [`compile`](FfmpegCommand::compile) to construct from media operations.
