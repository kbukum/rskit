...
//!
//! Renders several concurrent output streams as fixed-height "tiles" stacked in a live area at the bottom of the terminal,
//! each showing a bounded virtual terminal of one stream (via [`RegionScreen`]).
//! The tiles are an ephemeral progress peek:
//! rows that scroll off the top are dropped from the live view, not flushed to scrollback,
//! so a chatty stream does not flood the transcript.
//! Durable signal is emitted only at [`finish`](LiveConsole::finish) time — a one-line verdict — and,
//! for a failed region,
//! a bounded replay of its retained tail via [`finish_with_replay`](LiveConsole::finish_with_replay).
//!
//! The terminal mechanics (cursor positioning, redraw rate-limiting, width handling, resize) are delegated to `indicatif`'s multi-progress engine,
//! so this type only owns the tile layout, the bounded failure ring, and the verdict flush.
//! It is generic: callers feed labeled byte streams and get terminal rendering out,
//! with no domain types involved.
...
/// The string each tile indents its content by, beneath the region header.
/// The virtual terminal grid is sized to the tile width minus this indent's width
/// so a child's output fills the visible content area exactly, without being chopped.
...
pub struct LiveConfig {
    /// Maximum content rows a region tile may occupy — the height of its virtual terminal
    ...
    pub rows: usize,
    /// Terminal columns: the tile width.
    ...
    pub cols: usize,
    /// How many rows that scroll off a tile are retained per region for a failure replay.
...
impl LiveConfig {
    /// The inner virtual-terminal grid width:
    /// the tile width minus the content indent rendered under each region header.
    ///
    /// A child whose output is fed to the tile must be told its terminal is this wide (not the full tile width),
    /// so its own line wrapping matches the grid.
    /// Otherwise a full-width in-place progress redraw wraps at the grid edge, scrolls the short grid,
    /// and churns the retained failure tail with stale half-frames on every tick —
    /// which then surface in the bounded replay when a region fails.
    #[must_use]
...
struct Region {
    ...
    /// The tile is rendered to this many rows so it grows with output but never shrinks mid-run —
    /// output that clears (an in-place progress redraw) leaves the tile at its high-water height rather than collapsing
    /// and reflowing.
    high_water: usize,
}
...
/// [`feed`](Self::feed) raw bytes as they arrive,
/// then [`finish`](Self::finish) with a one-line verdict.
/// [`set_header`](Self::set_header) updates a status line pinned above the tiles.
...
impl LiveConsole {
    ...
    /// Create a console whose output is discarded — for tests
    /// and non-terminal runs where the live area must not render.
    #[must_use]
    ...
    fn with_target(target: ProgressDrawTarget, mut config: LiveConfig) -> Self {
        // Keep the grid and the renderer consistent: a tile always shows at least one content row
        // and its virtual terminal needs a real width.
    ...
    pub fn begin(&mut self, id: impl Into<String>, label: impl Into<String>) {
        ...
        // A fresh region shows only its header: it grows to fit content as bytes arrive,
        // so a silent or instantly-finishing unit never paints a tall block of blank rows.
    ...
    pub fn feed(&mut self, id: &str, bytes: &[u8]) {
        ...
        // Grow the tile to the tallest content it has shown, capped at the grid height,
        // and render exactly that many rows. `high_water` only rises,
        // so an in-place redraw that momentarily clears rows does not shrink the tile
        // and reflow the live area.
    ...
    ///
    /// For output that is not tied to a live region — a header banner,
    /// or a completed unit's buffered block on the rare path where a unit is not live-tailed.
    /// Returns any I/O error from the write.
    pub fn note(&self, line: impl AsRef<str>) -> std::io::Result<()> {
        self.multi.println(line.as_ref())
    }

    /// Drop every remaining region, then clear the live area and blank the header,
    /// leaving the console reusable.
    ///
    /// Remaining regions are units that never finished;
    /// their peeked output is discarded (durable signal is emitted at finish time).
    /// Returns any I/O error from the terminal clear.
    ...
    /// The rendered height of a region's tile in lines:
    /// its header plus the current high-water content rows.
    #[cfg(test)]
...
/// The number of rows up to and including the last non-blank one —
/// the height a tile must render to show all its current content. Trailing blank grid rows are excluded
/// so a tile grows only to the content it actually holds.
...
/// Render one tile as a labeled header line plus exactly `rows` indented content lines (padded with blanks),
/// each truncated to `cols` display columns. As with [`truncate`],
/// `cols == 0` disables truncation (used only by tests; the live console always resolves `cols` to at least 1).
/// `rows` is the caller's chosen tile height (its content high-water mark),
/// so the tile is exactly as tall as the content it currently holds rather than a fixed block.
...
/// Build a failed region's replay body:
/// its retained scrolled-off rows (oldest first) followed by the rows still on screen,
/// with trailing blank rows trimmed. The caller label-prefixes each line for scrollback attribution.
...
/// Truncate `line` to `width` display columns (ANSI- and width-aware),
/// leaving it untouched when `width` is `0`.
...
mod tests {
    ...
    fn scrolled_rows_are_retained_for_replay_not_lost() {
        ...
        // Only the last row stays on screen; the earlier rows scrolled off
        // but are retained for a possible failure replay.
        assert_eq!(
            console.retained("u1"),
    ...
    fn finish_with_replay_returns_the_transcript_for_an_end_of_run_epilogue() -> std::io::Result<()>
    {
        ...
        // The replayed body is returned un-prefixed and in order so a caller can retain it
        // and re-surface all failures together after the run.
    ...
    fn a_silent_region_renders_only_its_header() {
        ...
        console.begin("u1", "rust:core#test");
        // Nothing fed yet: the tile is a single header line, not a block of reserved blank rows.
    ...
    fn a_grown_tile_does_not_shrink_when_output_clears() {
        ...
        console.feed("u1", b"a\r\nb\r\nc");
        assert_eq!(console.tile_height("u1"), Some(4));
        // An in-place redraw that collapses to a single line keeps the tile at its high-water height instead of reflowing the live area.
    ...
    fn replay_line_clamped_to_width_never_wraps() {
        // A content row is sized to the tile's content area,
        // but the scrollback replay prefixes it with the full unit label —
        // the composed line must be clamped to the console width
        // so it never wraps into an orphan fragment.
