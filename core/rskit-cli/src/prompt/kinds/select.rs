...
use super::{
    Ask, cancelled, closed_input, focus_down, focus_up, non_interactive_error, notice, parse_index,
...
pub(crate) fn run(
    ...
    if choices.is_empty() {
        return Err(rskit_errors::AppError::invalid_input(
            "prompt",
            format!("select requires at least one choice: {}", ask.prompt),
        ));
    }
    ...
    if !ask.mode.is_interactive() {
        return default.map_or_else(
            || Err(non_interactive_error(ask.prompt)),
            |index| Ok(choices[index].id().clone()),
        );
    }

    if terminal.capabilities().is_key_driven() {
        key_driven(terminal, ask, choices, default)
    } else {
        line_driven(terminal, ask, choices, default)
    }
}
...
fn key_driven(
    ...
    with_raw_mode(terminal, |terminal| {
        let mut drawn = draw(terminal, ask, choices, cursor, default)?;
        loop {
            ...
            drawn = draw(terminal, ask, choices, cursor, default)?;
        }
    })
}
...
fn draw(
    ...
    for row in render::frame_rows(ask.style, choices, cursor, None, default) {
        terminal.write_line(&row)?;
    }
    terminal.write_line(&render::key_hint(ask.style, false))?;
    terminal.flush()?;
...
fn line_driven(
    ...
    terminal.write_line(&render::heading(ask.style, ask.prompt))?;
    for row in render::numbered_rows(ask.style, choices, false, default) {
        terminal.write_line(&row)?;
    }
    loop {
        let hint = default.map(|index| format!("[{}]", index + 1));
        write_answer(terminal, ask.style, hint.as_deref())?;
        ...
        if answer.is_empty() {
            ...
            notice(terminal, ask.style, "a choice is required")?;
            continue;
        }
