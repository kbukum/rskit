...
use super::{cancelled, closed_input, non_interactive_error, with_raw_mode};
...
pub fn run(
    ...
    if !mode.is_interactive() {
        let value = default.ok_or_else(|| non_interactive_error(prompt))?;
        if let Some(v) = validator {
            v.validate(value).map_err(|reason| {
                rskit_errors::AppError::invalid_input(
                    "prompt",
                    format!("default for {prompt} is invalid: {reason}"),
                )
            })?;
        }
    ...
    if terminal.capabilities().is_key_driven() {
        key_driven(terminal, style, prompt, default, validator)
    } else {
        line_driven(terminal, style, prompt, default, validator)
    }
}
...
fn key_driven(
    ...
    terminal.write_line(&heading(style, prompt, default))?;
    with_raw_mode(terminal, |terminal| {
        let mut buffer = String::new();
        let mut error: Option<String> = None;
        let mut drawn = draw(terminal, style, &buffer, error.as_deref())?;
        loop {
            ...
            drawn = draw(terminal, style, &buffer, error.as_deref())?;
        }
    })
}
...
fn line_driven(
    ...
    loop {
        terminal.write(&format!("{}: ", heading(style, prompt, default)))?;
        terminal.flush()?;
        ...
        match accept(line.trim(), default, validator) {
            Ok(Some(value)) => return Ok(value),
            Ok(None) => {
                let notice = style.palette().warn("a value is required").into_owned();
                terminal.write_line(&format!("  {notice}"))?;
            }
            Err(reason) => {
                let notice = style.palette().warn(&reason).into_owned();
                terminal.write_line(&format!("  {notice}"))?;
            }
        }
    }
