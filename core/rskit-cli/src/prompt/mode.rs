...
/// so a session is only interactive when **both** streams are terminals.
/// If stderr is redirected (`cmd 2>log`) the user would never see the question,
/// so the mode falls back to [`NonInteractive`] even when stdin is a TTY.
///
/// [`NonInteractive`]: PromptMode::NonInteractive
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum PromptMode {
    /// Interactive stdio (both stdin and stderr are terminals): render prompts and read live,
    /// typed answers.
    Interactive,
    /// Non-interactive stdio (CI, piped, or a redirected prompt sink): never block;
    /// resolve each question to its declared default.
    NonInteractive,
}
