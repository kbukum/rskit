...
pub trait ConfigSink: fmt::Debug + Send + Sync + 'static {
    ...
    /// # Errors
    ///
    /// Returns a typed [`AppError`](rskit_errors::AppError) (cause preserved) if the backend rejects the write
    /// or is unreachable.
    fn set(&self, key: &str, value: SecretString) -> AppResult<()>;

    /// Remove the value stored at `key`.
    ///
    /// Removing a missing key succeeds (idempotent), unless the backend distinguishes absence as an error.
    ///
    /// # Errors
    ///
    /// Returns a typed [`AppError`](rskit_errors::AppError) (cause preserved) if the backend rejects the removal
    /// or is unreachable.
    fn remove(&self, key: &str) -> AppResult<()>;

    /// Set many key/value pairs.
    ///
    /// The default applies each entry with [`set`](ConfigSink::set) in order
    /// and fails fast on the first error, leaving earlier writes applied. Adapters with native batch
    /// or transactional writes should override this for atomicity and efficiency.
    ///
    /// # Errors
    ///
