...
///
/// This is the canonical [`serde_json::Value`] tree shared across rskit (see
/// `rskit-codec`), regardless of the on-disk format the document was parsed from.
pub type RawValue = serde_json::Value;

/// A map of dynamic-keyed raw subtrees (for example `[ecosystems.<id>]`).
///
/// Each subtree is kept as an un-deserialized [`RawValue`] so a downstream
/// owner — a provider, plugin, or adapter — can parse it later with its own
/// `#[serde(deny_unknown_fields)]` schema. The strict top-level document rejects
/// unknown *reserved* keys while a [`RawTable`] field captures the open,
...
///
/// Honors `#[serde(deny_unknown_fields)]` on `T`. `field` names the subtree for
/// error reporting and is included in the returned [`AppError`] on failure.
