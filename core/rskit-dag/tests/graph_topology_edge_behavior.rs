...
#[tokio::test]
async fn deep_chain_20_levels_executes_in_order() {
...
#[tokio::test]
async fn wide_50_independent_nodes_all_execute() {
    let mut dag = Dag::new();
    for i in 0..50 {
...
#[tokio::test]
async fn double_diamond_topology_executes_dependencies_once() {
    // a→(b,c)→d→(e,f)→g
    let dag = Dag::new()
...
#[tokio::test]
async fn multi_fan_out_executes_all_children() {
    // root → c0..c9
...
#[tokio::test]
async fn multi_fan_in_waits_for_all_parents() {
    // r0..r9 → sink
    let mut dag = Dag::new();
    for i in 0..10 {
...
#[tokio::test]
async fn deep_narrow_chain_executes_all_nodes() {
...
#[tokio::test]
async fn complex_mixed_topology_executes_successfully() {
...
#[tokio::test]
async fn node_failure_propagates_error() {
...
#[tokio::test]
async fn root_failure_prevents_downstream_execution() {
    let dag = Dag::new()
...
#[tokio::test]
async fn failure_in_middle_of_chain_stops_downstream() {
    // a → b(fail) → c
    let dag = Dag::new()
        .add_node(TestNode::new("a", 1))
...
#[tokio::test]
async fn independent_node_failure_does_not_stop_unrelated_nodes() {
...
#[tokio::test]
async fn error_node_returns_clean_app_error() {
...
#[tokio::test]
async fn slow_node_observes_cancellation() {
    let cancel = CancellationToken::new();
    cancel.cancel();
...
#[tokio::test]
async fn all_nodes_receive_cancel_token() {
    let cancel = CancellationToken::new();
    cancel.cancel();
...
#[tokio::test]
async fn cycle_in_subgraph_is_rejected() {
...
#[tokio::test]
async fn large_ring_cycle_is_rejected() {
    // n0 → n1 → … → n9 → n0
    let mut dag = Dag::new();
    for i in 0..10 {
        dag = dag.add_node(TestNode::new(&format!("n{i}"), 1));
...
#[tokio::test]
async fn independent_nodes_execute_concurrently() {
...
#[tokio::test]
async fn diamond_branches_execute_in_parallel() {
...
#[tokio::test]
async fn large_parallel_graph_executes_with_bounded_duration() {
...
#[tokio::test]
async fn hundred_node_linear_chain_executes() {
    let mut dag = Dag::new();
    for i in 0..100 {
        dag = dag.add_node(TestNode::new(&format!("n{i}"), 1));
    }
...
#[tokio::test]
async fn hundred_independent_nodes_execute() {
    let mut dag = Dag::new();
    for i in 0..100 {
        dag = dag.add_node(TestNode::new(&format!("n{i}"), i));
    }
...
#[tokio::test]
async fn fifty_node_diamond_executes() {
    // root → m0..m47 → sink
