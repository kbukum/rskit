//! Behavioral tests for dependency container registration, resolution, and shutdown.
...
#[tokio::test]
async fn concurrent_eager_resolves_return_registered_instance() {
    struct SharedSvc {
        val: i32,
    }
...
#[tokio::test]
async fn concurrent_factory_resolves_create_independent_instances() {
...
#[tokio::test]
async fn singleton_closeable_closes_when_resolved_before_close() {
    let closed = Arc::new(AtomicU32::new(0));
    let c = Container::new();
    c.register_singleton_closeable({
        let closed = closed.clone();
        move || {
            Ok(Arc::new(MockCloseable {
                closed: Mutex::new(closed.load(Ordering::SeqCst) > 0),
            }))
        }
    });

    let resolved = c.resolve::<MockCloseable>().unwrap();
    assert!(!*resolved.closed.lock());
...
async fn close_aggregates_errors_without_short_circuiting() {
    ...
    let c = Container::new();
    c.register_closeable(Arc::new(Failing { ran: ran.clone() }));
    c.register_closeable(Arc::new(Failing { ran: ran.clone() }));
    c.register_closeable(Arc::new(Failing { ran: ran.clone() }));
...
#[test]
fn container_resolves_many_registered_types() {
    let c = Container::new();
...
#[test]
fn re_registering_eager_service_replaces_previous_value() {
    let c = Container::new();
    c.register(Arc::new(Config {
        db_url: "first".into(),
    }));
...
#[test]
fn re_registering_singleton_replaces_cached_instance() {
...
#[test]
fn re_registering_factory_replaces_previous_factory() {
    struct Svc {
        tag: String,
    }
...
#[test]
fn container_handles_large_number_of_registered_types() {
    let c = Container::new();
...
#[test]
fn singleton_resolves_return_same_arc_instance() {
...
#[test]
fn eager_registration_returns_same_arc_every_time() {
    let c = Container::new();
...
#[test]
fn eager_registration_can_be_replaced_by_factory() {
    struct ModeSvc {
        source: String,
    }
