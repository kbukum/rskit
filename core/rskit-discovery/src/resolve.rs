...
//!
//! [`resolve_addr`] resolves a service name to a `(host, port)` pair using the [`Discovery`] trait.
//! This is intended for one-shot infrastructure resolution at startup —
//! before connection pools are created — not for runtime load balancing.
...
/// Use this at bootstrap time to resolve infrastructure addresses (database, redis, kafka, etc.) before connection pools are created.
