...
#[test]
fn diff_stats_count_changed_files_and_lines() {
    let repo = helpers::TestRepo::init();
    repo.create_tag("v1");
...
#[test]
fn status_is_empty_for_clean_worktree() {
    let repo = helpers::TestRepo::init();
    let r = open(repo.path()).unwrap();
...
#[test]
fn index_entry_reads_staged_blob_identity() {
    let repo = helpers::TestRepo::init();
...
#[test]
fn file_at_reads_blob_from_revision() {
    let repo = helpers::TestRepo::init();
...
#[test]
fn file_at_returns_error_for_missing_path() {
    let repo = helpers::TestRepo::init();
    let r = open(repo.path()).unwrap();
...
#[test]
fn list_entries_returns_tree_and_blob_entries() {
    let repo = helpers::TestRepo::init();
...
#[test]
fn list_entries_can_scope_to_subdirectory() {
    let repo = helpers::TestRepo::init();
...
#[test]
fn tree_hash_changes_when_tree_changes() {
    let repo = helpers::TestRepo::init();
    repo.create_tag("v1");
    let r = open(repo.path()).unwrap();
...
#[test]
fn log_options_filter_by_count_path_author_and_time() {
    let repo = helpers::TestRepo::init();
...
#[test]
fn merge_base_and_is_ancestor_report_graph_relationships() {
    let repo = helpers::TestRepo::init();
...
#[test]
fn blame_reports_line_commit_history() {
    let repo = helpers::TestRepo::init();
...
#[test]
fn rev_parse_describe_grep_and_show_read_repository_objects() {
    let repo = helpers::TestRepo::init();
