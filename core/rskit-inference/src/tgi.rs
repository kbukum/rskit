...
//! modern OAI-compat path, which is the recommended production interface.
...
pub struct Config {
    /// Base URL of the TGI server (e.g. `http://localhost:8080`).
    pub base_url: String,

    /// Default model identifier (e.g. `tiiuae/falcon-7b-instruct`).
    ...
    /// Max new tokens for generation (default: 256).
...
pub fn register(registry: &mut Registry) -> Result<(), RegistryError> {
    let factory: Factory = Arc::new(|cfg| {
        ...
        Ok(Arc::new(
            TgiAdapter::new(config).map_err(|e| InferenceError::Decode(e.to_string()))?,
        ))
    });
...
mod tests {
    ...
    #[test]
    fn config_defaults() {
        let cfg: Config = serde_json::from_str(r#"{"base_url":"http://localhost:8080"}"#).unwrap();
        assert_eq!(cfg.model, "tgi");
