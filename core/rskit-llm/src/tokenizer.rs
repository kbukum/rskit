...
/// count. This is the canonical tokenization port owned by `rskit-llm`; exact
/// model-specific tokenizers are provided by `contrib/llm/*` adapters that
/// implement this trait.
pub trait TokenCounter: Send + Sync {
    /// Returns the number of tokens in `text`.
    ///
    /// An empty string yields `0`, and the count never decreases as text is
    /// appended.
...
mod tests {
    ...
    fn heuristic_known_values_are_stable() {
        ...
        assert_eq!(HeuristicTokenCounter.count("abcdefgh"), 2);
    }
    ...
    #[test]
    fn heuristic_is_monotonic_with_length() {
        let counter = HeuristicTokenCounter;
        let mut prev = 0;
        ...
        for _ in 0..64 {
            ...
            assert!(count >= prev, "count must not decrease as text grows");
            prev = count;
        }
    }
    ...
    fn heuristic_counts_chars_not_bytes() {
        // Four multi-byte characters must still count as ~one token, proving the
        // heuristic measures characters rather than UTF-8 bytes.
        assert_eq!(HeuristicTokenCounter.count("héllo"), 2);
    }
