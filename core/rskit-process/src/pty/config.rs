...
///
/// The child runs attached to a real controlling terminal,
/// so it renders output exactly as it would in an interactive shell — colors, progress bars,
/// and tty-gated formatting are preserved. Unlike the pipe-backed modes, a PTY has a single stream:
/// the child's stdout and stderr are merged in emission order.
///
/// The merged stream is delivered through the observer's **stdout** callbacks and,
/// when the output policy captures either stdout or stderr,
/// retained as the result's stdout (the two streams are merged, so either capture flag opts into keeping the combined output).
/// The child never writes to the result's stderr in this mode;
/// the only bytes that can appear there are synthetic termination diagnostics injected by the lifecycle layer (for example a note that the child was killed after a timeout or cancellation).
#[derive(Clone, Default)]
pub struct PtyIo {
    /// Standard input policy. Only [`InputPolicy::Closed`] and [`InputPolicy::Bytes`] are supported;
    ...
    /// Capture policy for the merged output stream. Either `capture_stdout`
