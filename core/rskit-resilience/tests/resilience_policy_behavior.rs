//! Behavioral tests for resilience policies, limiters, tower layers, and composition.
...
use rskit_resilience::{
    BackoffKind, Bulkhead, BulkheadConfig, BulkheadLayer, CbConfig, CbState, CircuitBreaker,
    CircuitBreakerLayer, ConstantBackoff, LinearBackoff, RateLimitLayer, RateLimiter,
    RateLimiterConfig, RetryError, RetryLayer, RetryPolicy, RetryPreset, TimeoutLayer,
};
...
#[test]
fn public_config_builders_validate_clone_and_format() {
    let acquired = Arc::new(AtomicUsize::new(0));
    let released = Arc::new(AtomicUsize::new(0));
    let rejected = Arc::new(AtomicUsize::new(0));

    let bulkhead = BulkheadConfig::new("builder-bulkhead", 2)
    ...
    assert_eq!(cloned.name, "builder-bulkhead");
    assert_eq!(cloned.max_concurrent, 2);
    assert_eq!(cloned.max_wait, Duration::from_millis(25));
    assert!(format!("{bulkhead:?}").contains("builder-bulkhead"));
    (cloned.on_acquire.as_ref().unwrap())();
    (cloned.on_release.as_ref().unwrap())();
    (cloned.on_reject.as_ref().unwrap())();
    assert_eq!(acquired.load(Ordering::SeqCst), 1);
    assert_eq!(released.load(Ordering::SeqCst), 1);
    assert_eq!(rejected.load(Ordering::SeqCst), 1);

    let rate_config = RateLimiterConfig::new("builder-rate", 5, 2)
        .with_per_second(10)
        .with_burst(4);
    rate_config.validate().unwrap();
    assert_eq!(rate_config.per_second, 10);
    assert_eq!(rate_config.burst, 4);
    assert!(RateLimiterConfig::new("bad-rate", 0, 1).validate().is_err());
    assert!(
        RateLimiterConfig::new("bad-burst", 1, 0)
            .validate()
            .is_err()
    );
    let limiter = RateLimiter::from_config(rate_config).unwrap();
    assert!(format!("{limiter:?}").contains("builder-rate"));
}

#[test]
fn retry_public_builders_presets_error_and_layer_accessors() {
    let source_error = AppError::connection_failed("backend");
    ...
    assert!(
        retry_error
            .to_string()
            .contains("all 3 retry attempts failed")
    );
    assert!(std::error::Error::source(&retry_error).is_some());

    let constant = ConstantBackoff::new(Duration::from_millis(7));
    assert_eq!(constant.delay, Duration::from_millis(7));
    ...
    let policy = RetryPolicy::standard()
        .with_retry_if(|error| error.code() == ErrorCode::ConnectionFailed)
        .with_on_retry({
    ...
    assert!(cloned.retry_if.is_some());
    assert!(cloned.on_retry.is_some());
    assert_eq!(cloned.jitter_seed, Some(42));
    ...
    assert_eq!(
        RetryPreset::ExternalService.policy().max_elapsed_time,
        Duration::from_secs(30)
    );
    assert!(RetryPolicy::new().with_max_attempts(0).validate().is_err());
    assert!(
        RetryPolicy::new()
            .with_backoff_factor(f64::INFINITY)
            .validate()
            .is_err()
    );

    let retry_layer = RetryLayer::new(fast.clone());
    assert_eq!(retry_layer.policy().max_attempts, fast.max_attempts);
    let rate_layer = RateLimitLayer::new(RateLimiter::new("layer-accessor", 10, 1).unwrap());
    assert!(format!("{:?}", rate_layer.limiter()).contains("layer-accessor"));
    let timeout_layer = TimeoutLayer::new(Duration::from_millis(123));
    assert_eq!(timeout_layer.timeout(), Duration::from_millis(123));
}
...
#[tokio::test]
async fn circuit_breaker_allows_concurrent_calls_while_closed() {
...
#[tokio::test]
async fn circuit_breaker_execute_after_reset_uses_clean_state() {
...
#[tokio::test]
async fn circuit_breaker_open_state_returns_service_unavailable_error() {
...
#[tokio::test]
async fn circuit_breaker_opens_on_failure_threshold_boundary() {
...
#[tokio::test]
async fn bulkhead_exactly_max_concurrent_calls_succeed() {
    let bh =
...
#[tokio::test]
async fn bulkhead_overflow_is_rejected_after_wait_timeout() {
    let bh =
...
#[tokio::test]
async fn bulkhead_slot_is_released_after_inner_error() {
    let bh = Bulkhead::new(
...
#[tokio::test]
async fn bulkhead_reports_available_and_in_use_slots() {
...
#[tokio::test]
async fn bulkhead_callbacks_run_in_acquire_then_release_order() {
...
#[tokio::test]
async fn bulkhead_reject_callback_runs_for_overflow() {
    let rejected = Arc::new(AtomicUsize::new(0));
...
#[tokio::test]
async fn bulkhead_bounds_concurrent_stress_to_configured_slots() {
