...
This crate owns the opinion-free, layer-zero building blocks of the
...
- **`RskitStreamExt`** — `futures::Stream` extension operators (`rmap`, `rfilter`,
  `rfan_out`, `rparallel`, `rbatch`, windowing, throttling, and more).
- **`collect` / `drain` / `for_each`** — terminal sink combinators.

Sequential, named-step workflow orchestration is a different concern and lives in
[`rskit-chain`](../rskit-chain) (typed step sequences with progress and cancellation).
