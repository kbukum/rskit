...
//! Consumers that must not miss events should treat any received item as a signal to re-run their full load pipeline.
...
impl<T> Broadcaster<T> {
    ...
    ///
    /// Each subscriber receives an independent clone. A subscriber whose buffer is full is retained
    /// but skips this event (bounded drop backpressure);
    /// a subscriber whose stream has been dropped is pruned.
