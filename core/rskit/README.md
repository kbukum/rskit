...
Pure facade package that re-exports `rskit-*` crates from the Rust crate name `rskit`.
It contains no implementation logic;
use it when an application wants one dependency entry point instead of importing each foundation crate directly.
...
The default dependency re-exports the domain-free foundation modules: `util`, `errors`, `fs`,
`config`, `logging`, `resilience`, `provider`, `pipeline`, `bootstrap`, `component`, `worker`,
and `validation`.
...
```toml
[dependencies]
rskit-suite = "0.2.0-alpha.5"
# or opt into higher-level modules/adapters:
# rskit-suite = { version = "0.2.0-alpha.5", features = ["server", "cli", "storage-s3"] }
```
