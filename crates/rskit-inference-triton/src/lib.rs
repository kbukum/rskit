...
impl TritonInference {
    ...
    async fn predict_authorized(
        ...
        let span = tracing::info_span!("inference.predict");
        span.record(semconv::SYSTEM, SYSTEM);
        span.record(semconv::OPERATION_NAME, operation.as_str());
        span.record(semconv::REQUEST_MODEL, request.model_name.as_str());
        span.record(semconv::REQUEST_MODEL_VERSION, model_version.as_str());
        span.record(semconv::REQUEST_ID, request_id.as_str());
