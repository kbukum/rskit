...
| Async / semantic-similarity metrics | `rskit-bench` owns the `AsyncMetric` port (I/O-backed scoring awaited after synchronous metrics) and `semantic_similarity`, an embedding-cosine metric over an injected `rskit-embedding` provider, with a model-identity-bearing result name and canonical `rskit-resilience` timeout/retry | Async, I/O-backed benchmark-metric port plus an embedding-similarity metric in every kit | Tracked in gokit (https://github.com/kbukum/gokit/issues/342) and pykit (https://github.com/kbukum/pykit/issues/144) |
