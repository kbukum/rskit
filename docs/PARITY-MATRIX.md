...
| Config/source/format handling | Serde-based typed configs; source/format detection delegates to `rskit-codec` (`select::codec_for_path`); domain helpers in `server` and `file` | Config loading/source precedence/masking centralized per kit | Aligned — format selection routed through the canonical `rskit-codec` selector (case-insensitive, extension-driven); source precedence owned by `config` |
