...
| L9 | bench / git / testutil | ✅ | ✅ | module presence aligned; rskit currently leads on CLI-backed signed annotated tags (`RefManager::create_signed_tag`), so sibling parity needs a follow-up capability check |
...
| Async / semantic-similarity metrics | `rskit-bench` owns the `AsyncMetric` port (I/O-backed scoring awaited after synchronous metrics) and `semantic_similarity`, an embedding-cosine metric over an injected `rskit-embedding` provider, with a comparison-safe result name that folds in both the embedding-model identity and the configured threshold, and canonical `rskit-resilience` timeout/retry; every threshold-dependent metric (`classification`, `fuzzy_match`, `semantic_similarity`) folds its cutoff into the metric identity, validates it as finite and in `[0, 1]` (typed `InvalidInput` otherwise), and records it as provenance rather than a directional value, so runs scored at different thresholds are never joined and diffed | Async, I/O-backed benchmark-metric port plus an embedding-similarity metric in every kit, with threshold folded into metric identity so runs at different cutoffs are never compared as equivalent | `AsyncMetric` + `semantic_similarity` tracked in gokit (https://github.com/kbukum/gokit/issues/342) and pykit (https://github.com/kbukum/pykit/issues/144); the threshold-in-identity comparison-safety for `classification`/`fuzzy_match`/`semantic_similarity` is an rskit hardening whose sibling propagation is not yet tracked in a dedicated issue |
| LLM-judge metric | `rskit-bench` owns `llm_judge`, an `AsyncMetric` grading predictions via an injected `rskit-llm` provider and a versioned `JudgePrompt` (canonical `rskit-ai` prompt template with semver identity), parsing the untrusted reply into a validated `JudgeVerdict` (malformed/out-of-range/non-JSON replies, over-long replies, and incomplete completions → typed error, no success-shaped fallback), with canonical `rskit-resilience` timeout and bounded concurrency; judge provider/model + prompt identity recorded in `RunProvenance` | Async LLM-judge benchmark metric with a versioned prompt, structured untrusted-output parsing, and judge/model provenance in every kit | Tracked in gokit (https://github.com/kbukum/gokit/issues/340) and pykit (https://github.com/kbukum/pykit/issues/142) |
