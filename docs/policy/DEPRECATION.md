...
1. A `#[deprecated]` attribute with `since` and `note` — recognised by `rustc`, `clippy`, IDEs,
   and `docs.rs`.
...
5. (Where helpful) a runtime `tracing::warn!` from the function's first call,
   gated by `std::sync::OnceLock<()>`, naming the replacement. This is optional —
...
and announced via GitHub Security Advisories and (where applicable) [RUSTSEC](https://rustsec.org/).
