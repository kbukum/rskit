...
// Untrusted input crosses a trust boundary when a peer's RFC 9457 error response is deserialized.
// Parsing arbitrary bytes must never panic,
// and a successfully parsed `ProblemDetail` must serialize
// and round-trip back through the deserializer without error.
fuzz_target!(|data: &[u8]| {
